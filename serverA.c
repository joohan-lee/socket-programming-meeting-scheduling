...
#define PORT "21119" // serverA UDP PORT NUMBER, 21000 + 3 digits of my ID
...
int main(){
    ...
    struct addrinfo *pM;
    struct addrinfo serverA_hints, *servAinfo;
    socklen_t addr_len;
    ...
    memset(&serverA_hints, 0, sizeof serverA_hints);
    serverA_hints.ai_family = AF_INET; // set to AF_INET to use IPv4
    serverA_hints.ai_socktype = SOCK_DGRAM;

    // get address info of serverM(UDP)
    ...
    if((rv = getaddrinfo(myIP, PORT, &serverA_hints, &servAinfo)) != 0){
        fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv)); 
        return 1;
    }
    ...
    int num_users;
    user_t* users = read_users("a.txt", &num_users);
    ...
    // Send the initial list of usernames to the main server(serverM).
    if ((numbytes = sendto(sockfd, str_all_usernames, strlen(str_all_usernames)+1, 0, 
        p->ai_addr, p->ai_addrlen) == -1)) {
        perror("Send failed at serverA");
        exit(EXIT_FAILURE);
    }
    ...
    for(p = servAinfo; p != NULL; p = p->ai_next) {
